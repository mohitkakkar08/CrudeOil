...
class DataFeedError(RuntimeError): pass
...
class FyersDataFeed:
 def __init__(self,access_token:str,socket_factory:SocketFactory|None=None)->None:
  self._access_token=access_token; self._socket_factory=socket_factory or _sdk_socket_factory; self._socket:DataSocket|None=None; self._diagnostic_code='SOCKET_NOT_STARTED'; self._subscription_ready=False
 @property
 def diagnostic_code(self)->str:return self._diagnostic_code
 def start(self,symbols:Sequence[str],on_tick:Callable[[Mapping[str,object]],None])->None:
  requested_symbols=list(symbols)
  def on_connect()->None:
   assert self._socket is not None
   self._socket.subscribe(symbols=requested_symbols,data_type='SymbolUpdate'); self._subscription_ready=True; self._diagnostic_code='SOCKET_SUBSCRIBED'
 ...
 def stop(self)->None:
  ...
  self._socket=None; self._subscription_ready=False
