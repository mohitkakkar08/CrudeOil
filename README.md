...
The worker writes the option chain in columns **A:AL**, mirroring the supplied CE / Strike / PE layout: CE market data, the central strike, then PE market data, timestamp, and both VWAP fields. It also writes the nearest unexpired BSE SENSEX futures contract in the summary block **L:Z**: OHLC, LTP, LTP change, volume, OI, OI change, OI change %, and VWAP. One FYERS websocket carries the index, India VIX, selected SENSEX future, and all current-expiry contracts. It supplies OHLC, LTP, volume, OI, OI change, and VWAP. A rate-limited, bounded FYERS Option Chain call supplements OI, OI change, IV, and Greeks (`Delta`, `Gamma`, `Vega`, `Theta`, `Rho`) for every option strike in the selected expiry.
...
The separate `MCX CrudeOil live option chain` workflow writes the `CrudeOil`,
`CrudeOil LTP Run`, and `CrudeOil Rolling Data` tabs in the CrudeOil workbook.
It discovers the current option expiry and current futures expiry separately
from FYERS `MCX_COM.csv`. Add `CRUDEOIL_GOOGLE_SHEET_ID` as a GitHub Actions
secret with the CrudeOil workbook ID; FYERS and Google-service-account secrets
are reused only when the same account has MCX market-data entitlement.

Use three workflow runs to stay within GitHub's six-hour hosted-job limit:
`morning` (09:00-15:00 IST), `afternoon` (15:00-21:00 IST), and `evening`
(21:00-23:30 IST). The workflow is deliberately manual-dispatch only until
the external scheduler is configured and tested.
